    ...
    for i = 1, #units do
        ...
        if CanHeal(u) then
            ...
            if UnitIsPet(u) then
                ...
                if not IsOneUnit('player', u) and HasBuff(protBuffsList, 1, u) then
                    h = h + 5
                end
                if not IsArena() and myHP < 50 and not IsOneUnit('player', u) and not (UnitThreat(u) == 3) then
                    h = h + 30
                end
            end
            tinsert(members, u)
...
function IsFriend(unit)
    if IsOneUnit(unit, 'player') then
        return true
    end
    if not UnitIsPlayer(unit) or not IsInteractUnit(unit) then
        return false
    end
...
function GetSameGroupUnit(unit)
    ...
    for i = 1, #group do
        if InOneUnit(unit, group[i]) then
            return group[i]
        end
    end
end
...
function HasClass(units, classes)
    ...
    if type(units) == 'table' then
        for i = 1, #units do
            local u = units[i]
            if checkClass(u, classes) then
                return true
            end
        end
    else
        if checkClass(units, classes) then
            return true
        end
    end
    return false
...
function UnitLostHP(unit)
    ...
    local lost = maxhp - hp
    if UnitThreatAlert(unit) == 3 then
        lost = lost * 1.5
    end
    return lost
end
...
------------------------------------------------------------------------------------------------------------------
function IsPvP()
    if IsValidTarget('target') and UnitIsPlayer('target') then
        return true
    end
    local inInstance, instanceType = IsInInstance()
    if InDuel() then
    ...
    if inInstance ~= nil and (instanceType == 'pvp' or instanceType == 'arena') then
        return true
    end
    return false
end
