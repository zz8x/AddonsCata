...
function UnitIsCasting(unit, last)
    ...
    if spell == nil or not startTime or not endTime then
        return nil
    end
...
function IsReady(left, checkGCD)
    if checkGCD == nil then
        checkGCD = false
    end
    if not checkGCD then
        local gcdLeft = GetGCDLeft()
        if (abs(left - gcdLeft) < 0.01) then
            return true
        end
    end
...
function InInteractRange(unit)
    ...
    if not IsInteractUnit(unit) then
        return false
    end
...
local function UpdateIsCast(event, ...)
    ...
    if spell and unit == 'player' then
        ...
        if event == 'UNIT_SPELLCAST_SUCCEEDED' and castInfo.StartTime and castInfo.StartTime > 0 then
...
function GetLastSpellError(spellName, t)
    ...
    if t and lastTime and (GetTime() - lastTime > t) then
        return nil
    end
...
function CanUseSpell(spellName, target)
    ...
    if dump then
        print('Пытаемся прожать', spellName, 'на', target)
    end

    -- Не мешаем выбрать область для спела (нажат вручную)
    if SpellIsTargeting() then
        if dump or true then
            print('Ждем выбор цели, не можем прожать', spellName)
        end
        return false
    end
    ...
    if IsPlayerCasting() then
        if dump then
            print('Кастим, не можем прожать', spellName)
        end
        return false
    end
    ...
    if not name then
        if Debug then
            error('Спел [' .. spellName .. '] не найден!')
        end
        return false
    end
    ...
    if IsSpellInUse(spellName) then
        if dump then
            print('Уже прожали, SPELL_SENT пошел, не можем больше прожать', spellName)
        end
        return false
    end
    ...
    if not usable then
        if dump then
            print('Не usable, не можем прожать', spellName)
        end
        return false
    end
    if nomana then
        if dump then
            print('Не достаточно маны, не можем прожать', spellName)
        end
        return false
    end
    ...
    if not IsReadySpell(spellName, true) then
        if dump then
            print('Не готово, не можем прожать', spellName)
        end
        return false
    end
    ...
    if target == nil and IsHarmfulSpell(spellName) then
        target = 'target'
    end
    -- проверяем что цель в зоне досягаемости
    if not InRange(spellName, target) then
        if dump then
            print(target, ' - Цель вне зоны досягаемости, не можем прожать', spellName)
        end
        return false
    end
