...
local unitWeights = {}
local friendTargets = {}
local function compareUnits(u1, u2)
    return unitWeights[u1] < unitWeights[u2]
end
local function getTargetWeight(t)
    ...
    if IsOneUnit('focus', t) then
        w = 3.1
    end
    if IsOneUnit('target', t) then
        w = 3.2
    end
    if IsOneUnit('mouseover', t) then
        w = 3.3
    end
...
local targetWeights = {}
local function compareTargets(t1, t2)
    return targetWeights[t1] < targetWeights[t2]
end
...
local function UpdateIdle(elapsed)
    ...
    if UnitIsDeadOrGhost('player') or UnitIsCharmed('player') or not UnitPlayerControlled('player') then
        return
    end
...
local function UpdateCombatLogFix()
    if InCombatLockdown() and GetTime() - CombatLogTimer > 15 and GetTime() - CombatLogResetTimer > 30 then
...
if HarmfulCastingSpell == nil then
    HarmfulCastingSpell = {}
end
...
local function updateDebugStats()
    if TimerLess('DebugFrame', 2) then
        return
    end
